...
async function loadData() {
  try {
    ...
    const [estadoRes, sistemaRes, historyRes] = await Promise.all([
      fetch(endpoint("estado"), { cache: "no-store" }),
      fetch(endpoint("sistema"), { cache: "no-store" }),
      fetch(endpoint("historial"), { cache: "no-store" })
    ]);

    if (!estadoRes.ok) throw new Error(`Firebase estado ${estadoRes.status}`);
    if (!sistemaRes.ok) throw new Error(`Firebase sistema ${sistemaRes.status}`);

    const estado = await estadoRes.json();
    const sistema = await sistemaRes.json();
    const history = historyRes.ok ? await historyRes.json() : {};
    const data = { estado: estado || {}, sistema: sistema || {} };
